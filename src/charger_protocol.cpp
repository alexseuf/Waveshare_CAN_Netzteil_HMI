...
bool ChargerProtocol::decodeStatus(const twai_message_t &m, PowerSupplyState &state) {
  ...
  // Ein erfolgreich dekodiertes Status-Telegramm beweist unmittelbar,
  // dass die CAN-Kommunikation funktioniert. Statusbits des Netzteils
  // beschreiben Geräte-/Batteriefehler und dürfen den CAN-Linkzustand
  // nicht dauerhaft auf RECOVERING halten.
  state.linkState = CanLinkState::Online;
  state.recoveryGoodFrames = 3;
