...
void setLed(lv_obj_t *obj, bool active, bool faultStyle) { lv_obj_set_style_bg_color(obj, active ? (faultStyle ? red() : green()) : grey(), 0); }
void setFloat(lv_obj_t *obj, float value, uint8_t decimals, const char *unit) { char number[24], text[32]; dtostrf(value,1,decimals,number); snprintf(text,sizeof(text),"%s %s",number,unit); lv_label_set_text(obj,text); }
void formatClock(uint32_t ms, char *dest, size_t size) { const uint32_t total=ms/1000UL; snprintf(dest,size,"%02lu:%02lu:%02lu",static_cast<unsigned long>(total/3600UL),static_cast<unsigned long>((total/60UL)%60UL),static_cast<unsigned long>(total%60UL)); }
void showPage(int index) { currentPage=index; for(int i=0;i<5;++i){ if(i==index)lv_obj_clear_flag(pages[i],LV_OBJ_FLAG_HIDDEN);else lv_obj_add_flag(pages[i],LV_OBJ_FLAG_HIDDEN); lv_obj_set_style_bg_color(nav[i],i==index?orange():darkGrey(),0);} DebugLog::log(DebugLog::UI,"[UI] Seite %d\n",index); }
...
void actionEvent(lv_event_t *e){ const int id=static_cast<int>(reinterpret_cast<intptr_t>(lv_event_get_user_data(e))); switch(id){case 1:adjust(.1f,0,"Spannung+");break;case 2:adjust(-.1f,0,"Spannung-");break;case 3:adjust(0,.1f,"Strom+");break;case 4:adjust(0,-.1f,"Strom-");break;case 5:state->chargingEnabled=true;CanDriver::sendCommand(*state);break;case 6:state->chargingEnabled=false;CanDriver::sendCommand(*state);break;case 20:Settings::resetDefaults(*state);break;case 21:state->resetEnergy();break;} }
...
void Ui::begin(PowerSupplyState &s){
  using namespace UiInternal;
  state=&s;
  DebugLog::printf("[UI-BOOT] Start heap=%u psram=%u\n",ESP.getFreeHeap(),ESP.getFreePsram());
  lv_obj_t *screen=lv_screen_active(); lv_obj_set_style_bg_color(screen,bg(),0);
  ...
  DebugLog::printf("[UI-BOOT] Haupt heap=%u\n",ESP.getFreeHeap()); makeMain(pages[0]);
  DebugLog::printf("[UI-BOOT] Scope heap=%u\n",ESP.getFreeHeap()); makeScope(pages[1]);
  DebugLog::printf("[UI-BOOT] Log heap=%u\n",ESP.getFreeHeap()); makeLog(pages[2]);
  DebugLog::printf("[UI-BOOT] Info heap=%u\n",ESP.getFreeHeap()); makeInfo(pages[3]);
  DebugLog::printf("[UI-BOOT] Settings heap=%u\n",ESP.getFreeHeap()); makeSettings(pages[4]);
  DebugLog::printf("[UI-BOOT] Fertig heap=%u psram=%u\n",ESP.getFreeHeap(),ESP.getFreePsram());
  showPage(0);
}
void Ui::update(const PowerSupplyState &s){ using namespace UiInternal;const uint32_t now=millis();char clock[32];formatClock(now,clock,sizeof(clock));char uptime[40];snprintf(uptime,sizeof(uptime),"Uptime %s",clock);lv_label_set_text(clockLabel,uptime); updateMain(s,now);updateScope(s,now);if(currentPage==2&&!logPaused)updateLog();if(currentPage==3){updateInfo(s,now);if(infoCanVisible()&&!canPaused)updateCan();}if(currentPage==4)updateSettings(now); }
