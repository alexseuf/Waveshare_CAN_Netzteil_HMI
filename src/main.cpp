...
void setup() {
  ...
  DebugLog::printf("[DEBUG] UART TX=GPIO%d RX=GPIO%d, %lu Baud\n", AppConfig::PIN_DEBUG_TX, AppConfig::PIN_DEBUG_RX, static_cast<unsigned long>(AppConfig::SERIAL_BAUD));
  ...
  if (!Display::begin()) { DebugLog::println("[BOOT] DISPLAY FEHLER"); while (true) delay(1000); }
...
void loop() {
  ...
  if (now - uiMs >= 100) { uiMs = now; Ui::update(state); Ui::updateTouchDiag(lastTouch); }
  ...
  if (now - healthMs >= 5000) {
    ...
    DebugLog::printf("[HEALTH] heap=%u intFree=%u intLargest=%u psTotal=%u psFree=%u psLargest=%u touch=%s CAN=%s RX=%s age=%lu ms status=0x%02X WiFi=%s\n",
                     ESP.getFreeHeap(),(unsigned)intFree,(unsigned)intLargest,(unsigned)psTotal,(unsigned)psFree,(unsigned)psLargest,
                     Touch::online()?"OK":"FEHLER",state.canStarted?"OK":"FEHLER",state.online(now)?"OK":"TIMEOUT",
                     state.lastRxMs ? static_cast<unsigned long>(now-state.lastRxMs) : 0UL,state.status,wifiState.c_str());
  }
  Display::task();
