...
static lv_obj_t *card(lv_obj_t *parent,const char *icon,const char *title,const char *description,int x,lv_color_t accent,SettingsPage target){
  ...
  lv_obj_t *d=makeLabel(b,description,&ui_font_de_14,lightGrey());lv_label_set_long_mode(d,LV_LABEL_LONG_WRAP);lv_obj_set_width(d,198);lv_obj_set_style_text_align(d,LV_TEXT_ALIGN_CENTER,0);lv_obj_align(d,LV_ALIGN_TOP_MID,0,102);
...
static lv_obj_t *backButton(lv_obj_t *parent){
  lv_obj_t *b=makeButton(parent,LV_SYMBOL_LEFT " ZURÜCK",8,5,130,36,darkGrey(),&ui_font_de_14);
...
void makeSettings(lv_obj_t *parent){
  ...
  lv_obj_t *title=makeLabel(settingsOverview,LV_SYMBOL_SETTINGS "  EINSTELLUNGEN",&ui_font_de_24,lv_color_white());lv_obj_align(title,LV_ALIGN_TOP_MID,0,5);
  ...
  overviewMetrics=makeLabel(settingsOverview,"",&ui_font_de_14,lightGrey());lv_obj_set_pos(overviewMetrics,15,302);lv_obj_set_size(overviewMetrics,730,70);lv_obj_set_style_bg_color(overviewMetrics,lv_color_hex(0x0D1823),0);lv_obj_set_style_bg_opa(overviewMetrics,LV_OPA_COVER,0);lv_obj_set_style_border_color(overviewMetrics,border(),0);lv_obj_set_style_border_width(overviewMetrics,1,0);lv_obj_set_style_radius(overviewMetrics,10,0);lv_obj_set_style_pad_all(overviewMetrics,14,0);

  debugPage=lv_obj_create(box);lv_obj_remove_style_all(debugPage);lv_obj_set_size(debugPage,760,402);lv_obj_set_pos(debugPage,0,0);lv_obj_clear_flag(debugPage,LV_OBJ_FLAG_SCROLLABLE);backButton(debugPage);lv_obj_t *dh=makeLabel(debugPage,"DEBUG SETTINGS",&ui_font_de_24,blue());lv_obj_align(dh,LV_ALIGN_TOP_MID,0,8);
  ...
  lv_obj_t *d=lv_checkbox_create(left);lv_checkbox_set_text(d,"Ausgabe auf Display");lv_obj_set_pos(d,12,175);lv_obj_set_style_text_font(d,&ui_font_de_14,0);if(DebugLog::displayEnabled())lv_obj_add_state(d,LV_STATE_CHECKED);lv_obj_add_event_cb(d,outOption,LV_EVENT_VALUE_CHANGED,(void*)1);
  lv_obj_t *u=lv_checkbox_create(left);lv_checkbox_set_text(u,"Ausgabe über UART");lv_obj_set_pos(u,12,208);lv_obj_set_style_text_font(u,&ui_font_de_14,0);if(DebugLog::uartEnabled())lv_obj_add_state(u,LV_STATE_CHECKED);lv_obj_add_event_cb(u,outOption,LV_EVENT_VALUE_CHANGED,(void*)2);
  ...
  lv_obj_t *right=lv_obj_create(debugPage);lv_obj_set_pos(right,378,52);lv_obj_set_size(right,374,330);stylePanel(right,10);makeLabel(right,"Systemdiagnose",&ui_font_de_18,lv_color_white());debugMetrics=makeLabel(right,"",&ui_font_de_16,lightGrey());lv_obj_set_pos(debugMetrics,12,42);lv_obj_set_size(debugMetrics,340,250);

  wifiPage=lv_obj_create(box);lv_obj_remove_style_all(wifiPage);lv_obj_set_size(wifiPage,760,402);lv_obj_set_pos(wifiPage,0,0);backButton(wifiPage);lv_obj_t *wh=makeLabel(wifiPage,LV_SYMBOL_WIFI "  WIFI",&ui_font_de_24,green());lv_obj_align(wh,LV_ALIGN_TOP_MID,0,8);lv_obj_t *wp=lv_obj_create(wifiPage);lv_obj_set_pos(wp,90,80);lv_obj_set_size(wp,580,230);stylePanel(wp,12);lv_obj_t *wt=makeLabel(wp,"WiFi-Funktion wird in Schritt 2 aktiviert.\n\nGeplant:\n• Netzwerksuche und Zugangsdaten\n• Status, Signalstärke und IP-Adresse\n• NTP-Zeitsynchronisation\n• Access-Point-Fallback\n\nCAN, Display und Ladebetrieb laufen unabhängig weiter.",&ui_font_de_16,lightGrey());lv_obj_set_pos(wt,25,22);

  mqttPage=lv_obj_create(box);lv_obj_remove_style_all(mqttPage);lv_obj_set_size(mqttPage,760,402);lv_obj_set_pos(mqttPage,0,0);backButton(mqttPage);lv_obj_t *mh=makeLabel(mqttPage,"MQTT / HOME ASSISTANT",&ui_font_de_24,lv_color_hex(0xA060FF));lv_obj_align(mh,LV_ALIGN_TOP_MID,0,8);lv_obj_t *mp=lv_obj_create(mqttPage);lv_obj_set_pos(mp,90,80);lv_obj_set_size(mp,580,230);stylePanel(mp,12);lv_obj_t *mt=makeLabel(mp,"MQTT-Funktion wird in Schritt 3 aktiviert.\n\nGeplant:\n• Broker, Port und Zugangsdaten\n• Topic-Präfix und Client-ID\n• Home-Assistant-Discovery\n• Availability / Last Will\n• optionale, geschützte Fernsteuerung\n\nFernsteuerung bleibt standardmäßig deaktiviert.",&ui_font_de_16,lightGrey());lv_obj_set_pos(mt,25,22);
