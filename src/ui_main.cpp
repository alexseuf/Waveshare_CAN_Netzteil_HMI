...
void makeMain(lv_obj_t *parent){
  ...
  lv_obj_t *et=makeLabel(mode,"ENERGIE",&ui_font_de_14,blue());lv_obj_set_pos(et,114,3);energyLabel=makeLabel(mode,"0.000\nkWh",&ui_font_de_24,lv_color_white());lv_obj_set_pos(energyLabel,101,31);lv_obj_set_width(energyLabel,
    108);lv_obj_set_style_text_align(energyLabel,LV_TEXT_ALIGN_CENTER,0);
  ...
  lv_obj_t *summary=lv_obj_create(parent);lv_obj_set_pos(summary,574,354);lv_obj_set_size(summary,206,68);stylePanel(summary);canSummary=makeLabel(summary,"CAN getrennt\n250 kbit/s",&ui_font_de_14,lightGrey());lv_obj_center(canSummary);lv_obj_set_style_text_align(canSummary,LV_TEXT_ALIGN_CENTER,0);
}
void updateMain(const PowerSupplyState &s,uint32_t now){
  setFloat(vSet,s.voltageSet,1,"V");setFloat(aSet,s.currentSet,1,"A");const bool online=s.online(now);
  ...
  char summary[64];snprintf(summary,sizeof(summary),"CAN %s\n250 kbit/s",s.linkStateText());lv_label_set_text(canSummary,summary);lv_obj_set_style_text_color(canSummary,online?green():(s.linkState==CanLinkState::Recovering?orange():red()),0);
}
}
